...
extension UIBarButtonItem{
    ...
    convenience init(title: String, fontSize: CGFloat = 16, target: Any?, action: Selector, isBack :Bool = false){
        let button = UIButton.textButton(withTitle: title, andWithFontSize: fontSize, andWithNormalColor: HelperCommonValues.SCNaviBarTintColor, andWithHighlight: UIColor.darkGray)
