...
private extension SCOAuthViewController{
    ...
    func setupWebView(){
        ...
        let urlString = "https://\(region).battle.net/oauth/authorize?access_type=online&client_id=\(HelperCommonValues.SCClientId)&redirect_uri=\(HelperCommonValues.SCRedirectURL)&response_type=code"
...
extension SCOAuthViewController: WKNavigationDelegate{
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        if navigationAction.request.url?.absoluteString.hasPrefix(HelperCommonValues.SCRedirectURL) == false {
        ...
        SCNetworkManager.shared.getAccessToken(code: code) { (isSuccess) in
            if !isSuccess{
                ...
                NotificationCenter.default.post(name: NSNotification.Name(HelperCommonValues.SCUserSuccessLoginNotification), object: nil)
                self.goBack()
            }
            decisionHandler(WKNavigationActionPolicy.cancel)
        }
